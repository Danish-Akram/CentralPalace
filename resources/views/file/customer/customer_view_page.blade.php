...
<div>
<form action="{{route('customer_update_data', $customer->id)}}" method="POST" enctype="multipart/form-data" autocomplete="off">
{{csrf_field()}}
...
<div class="row" style="">
   <span>
<label  > Code : </label>
<input  type="text" name="code" id="code" tabindex="1" value="{{ $customer->tcstcod }}" class="form-control" maxlength="10" readonly  onkeyup="autoMove(this,'status')" pattern="[0-9]{2}-[0-9]{2}-[0-9]{4}" title="00-00-0000">
</span>
<span style="margin-left: 21px">
    <label  >STATUS : </label>
    <select name="status" id="status" style="height:20px;" disabled tabindex="2" class="form-control " onkeyup="autoMove(this,'description')">
        <option value="A" @if( $customer->tcststs == "A") ? selected : null @endif> ACTIVE</option>
        <option value="N" @if( $customer->tcststs == "N") ? selected : null @endif> NOT ACTIVE</option>
      </select>
</select>
</span>
</div>
...
<div class="row">
<label  > Name : </label>
<input  type="text" name="name" tabindex="3" readonly value="{{ $customer->tcstnam }}" id="name" class="form-control " size="60" value="" maxlength = "40" onkeyup="autoMove(this,'urdu_description')">
</div>
<div id="description_error"></div>

    <div>
        <label> Address 1 :</label>
        <input  type="text" name="tempadd1" readonly value="{{ $customer->tcstadd1 }}" tabindex="4" class="form-control"  maxLength="40" size="60"value="">
    </div>
    <div>
        <span>
            <label> Address 2 :</label>
            <input  type="text" name="tempadd2" readonly value="{{ $customer->tcstadd2 }}" tabindex="5" class="form-control"  maxLength="40" size="60"value="">    
        </span>
    </div>
    <div>
        <label  > NIC : </label>
        <input  type="text" name="nic" id="nic" readonly value="{{ $customer->tcstnic }}" tabindex="6" class="form-control" maxlength = "15" size= "" onkeypress='validate(event)' onkeyup="autoMove(this,'submit')">
        </div> 
    <div class="row">
        <label  > Mobile : </label>
        <input  type="text" name="mobile" readonly value="{{ $customer->tphnnum }}" tabindex="7" class="form-control" id="mobile"  value="" size= ""  maxlength = "11" onkeyup="autoMove(this,'debit_amount')" onkeypress='validate(event)'>
        </div>
        <div class="row">
            <label  > Email : </label>
            <input  type="email" name="email" readonly value="{{ $customer->tcstema }}" class="form-control" tabindex="8" id="email"  value="" size="60" maxlength = "40" onkeyup="autoMove(this,'mobile')" style="text-transform: lowercase !important">
            </div>
